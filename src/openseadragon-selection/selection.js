    ...
    */
    $.Selection = function ( options ) {
        ...
        $.extend( true, this, {
            ...
            handleStyle: {
                ...
                margin:     '-3px 0 0 -3px',
                background: '#000',
                cursor:     ['ns-resize', 'ew-resize', 'ns-resize', 'ew-resize'],
            ...
            cornersStyle: {
                ...
                shift:      '-2.5px',
                background: '#000',
                cursor:     ['nwse-resize', 'nesw-resize', 'nwse-resize', 'nesw-resize'],
                border:     '1px solid #ccc'
            }
        }, options );
        ...
        if (!this.element) {
            ...
            this.element.style.background = this.selectionStyle.background;
            this.element.style.cursor = this.selectionStyle.cursor;
        ...
        for (var i = 0; i < 4; i++) {
            ...
            handle.style.cursor     = this.handleStyle.cursor[i];
            new $.MouseTracker({
            ...
            corners[i].style.cursor     = this.cornersStyle.cursor[i];
            new $.MouseTracker({
                element:     corners[i],
        ...
        corners[0].style.top = this.cornersStyle.shift;
        corners[0].style.left = this.cornersStyle.shift;
        corners[1].style.top = this.cornersStyle.shift;
        corners[1].style.right = this.cornersStyle.shift;
        corners[2].style.bottom = this.cornersStyle.shift;
        corners[2].style.right = this.cornersStyle.shift;
        corners[3].style.bottom = this.cornersStyle.shift;
        corners[3].style.left = this.cornersStyle.shift;
        ...
        this.innerTracker = new $.MouseTracker({
            ...
            // keyDownHandler:     $.delegate( this, onInsideKeyPress ),
        ...
        this.viewer.addHandler('change', this.onChange);
    ...
    $.extend( $.Selection.prototype, $.ControlDock.prototype, /** @lends OpenSeadragon.Selection.prototype */{
        ...
        changeHandler: function () {
            if (this.onChange && this.rect) {
                ...
                this.viewer.raiseEvent('change', result);
            }
        },
    ...
    function onInsideDrag(e) {
        ...
        if (e.shift) {
            this.rect.x += delta.y
            this.rect.y += delta.y
            this.rect.width -= delta.y * 2
            this.rect.height -= delta.y * 2
            if (this.restrictToImage && !this.rect.fitsIn(new $.Rect(0, 0, bounds.width, bounds.height))) {
                this.rect.x -= delta.y
                this.rect.y -= delta.y
                this.rect.width += delta.y * 2
    ...
    function onBorderDrag(border, e) {
        var delta = e.delta, shift = (e.shift) ? 1 : 0;
        ...
        switch (border) {
            case 0:
                this.rect.y += delta.y;
                this.rect.height -= delta.y * (1 + shift);
                break;
            case 1:
                this.rect.x -= shift * delta.x;
                this.rect.width += delta.x * (1 + shift);
                break;
            case 2:
                this.rect.y -= shift * delta.y
                this.rect.height += delta.y * (1 + shift);
                break;
            case 3:
                this.rect.x += delta.x;
